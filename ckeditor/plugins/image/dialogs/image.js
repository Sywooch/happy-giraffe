...
(function()
{
	var imageDialog = function( editor, dialogType )
	{
			...
			if ( dialog.lockRatio )
			{
				...
				if ( oImageOriginal.getCustomData( 'isReady' ) == 'true' )
				{
					if ( this.id == 'txtHeight' )
					{
						...
						if ( !isNaN( value ) )
							dialog.setValueOf( 'Upload', 'txtWidth', value );
					}
					else		//this.id = txtWidth.
					{
						...
						if ( !isNaN( value ) )
							dialog.setValueOf( 'Upload', 'txtHeight', value );
					}
				}
			}
		...
		{
			if ( !dialog.getContentElement( 'Upload', 'ratioLock' ) )
				return null;
			...
			if ( value == 'check' )
			{
				...
				{
					var width = dialog.getValueOf( 'Upload', 'txtWidth' ),
						height = dialog.getValueOf( 'Upload', 'txtHeight' ),
		...
		var resetSize = function( dialog )
		{
			...
			if ( oImageOriginal.getCustomData( 'isReady' ) == 'true' )
			{
				var widthField = dialog.getContentElement( 'Upload', 'txtWidth' ),
					heightField = dialog.getContentElement( 'Upload', 'txtHeight' );
		...
		return {
			...
			onShow : function()
			{
				...
				if ( !CKEDITOR.tools.trim( this.getValueOf( 'Upload', 'txtUrl' ) ) )
				{
			...
			onLoad : function()
			{
				...
				if ( this.getContentElement( 'Upload', 'ratioLock' ) )
				{
									...
									[
										{
											...
											'<div id="' + imagePreviewBoxId + '" class="ImagePreviewBox"><a href="javascript:void(0)" target="_blank" onclick="return false;" id="' + previewLinkId + '">'+
											'<img id="' + previewImageId + '" alt="" /></a></div></div>'
										}
									]
								}
							]

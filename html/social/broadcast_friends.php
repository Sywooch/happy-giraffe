<!DOCTYPE html>
<!--[if lt IE 8]>      <html class="top-nav-fixed ie7"> <![endif]-->
<!--[if IE 8]>         <html class="top-nav-fixed ie8"> <![endif]-->
<!--[if IE 9]>         <html class="top-nav-fixed ie9"> <![endif]-->
<!--[if gt IE 9]><!--> <html class="top-nav-fixed"> <!--<![endif]-->
<head>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/block/global/head.php'; ?>

</head>
<body class="body-gray">

	<?php include $_SERVER['DOCUMENT_ROOT'].'/block/global/top-line-menu.php'; ?>
	
<div class="layout-container">
	<div class="layout-wrapper">
		<?php include $_SERVER['DOCUMENT_ROOT'].'/block/global/header-new.php'; ?>
		
		<div id="content" class="clearfix">
			
			<div id="broadcast" class="broadcast-all">
				
				<div class="broadcast-title-box">
					<h1><i class="icon-boradcast"></i> Что нового</h1>
					<ul class="broadcast-menu">
						<li>
							<a href="">Прямой эфир</a>
						</li>
						<li>
							<a href="">В клубах</a>
							<ul class="broadcast-menu_drop">
								<li class="active"><a href="">В моих</a></li>
								<li><a href="">Во всех</a></li>
							</ul>
						</li>
						<li>
							<a href="">В блогах</a>
							<ul class="broadcast-menu_drop">
								<li><a href="">В моих</a></li>
								<li><a href="">Во всех</a></li>
							</ul>
						</li>
						<li class="active">
							<a href="">У друзей</a>
						</li>
					</ul>
				</div>
				
				<div class="content-cols clearfix">
				
						<div class="masonry-news-list">
						<script>
  $(function(){

        $(".masonry-news-list").masonry({
          itemSelector : '.masonry-news-list_item',
          columnWidth: 240,
          isAnimated: false,
          animationOptions: { queue: false, duration: 500 }
        });

  })
						</script>
							<ul>
							
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила комментарий  в клубе </p>
										</div>
									</div>
									<div class="masonry-news-list_comment">
										<div class="masonry-news-list_comment-text">
											Проблема в том, что на 2006-ой год игры еще толком не было, Роды — отправная точка для самых важных изменений и судьбоносных. Осень. Я ее люблю, но когда начинается непонятная погода, хочется лезть на стенку. Сегодня на улице вот +8, вроде тепло, но вот туман.
										</div>
									</div>
									<div class="masonry-news-list_meta-info clearfix">к записи</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука 123 123</a>
									</h3>
									<div class="masonry-news-list_meta-info clearfix">
										
										<div class="meta">
											<div class="views"><span class="icon" href="#"></span> <span>11265</span></div>
											<div class="comments">
												<a class="icon" href="#"></a>
												<a href="">155</a>
											</div>
										</div>
										                                                 я
										<a href="" class="textdec-onhover">Вступить <br />в беседу!</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item blog">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила комментарий  в блоге </p>
										</div>
									</div>
									<div class="masonry-news-list_comment">
										<div class="masonry-news-list_comment-text">
											<a href="">Проблема в том, что на 2006-ой год игры еще толком не было... </a>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука 123 123</a>
									</h3>
									<div class="masonry-news-list_meta-info clearfix">
										
										<div class="meta">
											<div class="views"><span class="icon" href="#"></span> <span>5</span></div>
											<div class="comments">
												<a class="icon" href="#"></a>
												<a href="">5</a>
											</div>
										</div>
										
										<a href="" class="textdec-onhover">Вступить <br />в беседу!</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила запись</p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука. Раннее развитие и обучение.</a>
									</h3>
									<div class="clearfix">
										<a href="" class="club-category manwoman">Раннее развитие и обучение.</a>
									</div>
									<div class="masonry-news-list_content">
										<a href=""><img src="/images/example/gallery_album_img_10.jpg" alt="" /></a>
										<p>Роды — отправная точка для самых важных изменений и судьбоносных... <a href="" class="all">Читать</a></p>
									</div>
									
									<div class="masonry-news-list_meta-info clearfix">
										<div class="meta">
											<div class="views"><span class="icon" href="#"></span> <span>265</span></div>
											<div class="comments">
												<a class="icon" href="#"></a>
												<a href="">152</a>
											</div>
										</div>
										
										<a href="" class="textdec-onhover">Добавить <br />комментарий</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item blog">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила запись </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука. Раннее развитие и обучение.</a>
									</h3>
									<div class="clearfix">
										<a class="sub-category" href=""><span class="icon-blog"></span>Личный блог</a>
									</div>
									<div class="masonry-news-list_content">
										<a href=""><img src="/images/example/gallery_album_img_10.jpg" alt="" /></a>
										<p>Роды — отправная точка для самых важных изменений и судьбоносных... <a href="" class="all">Читать</a></p>
									</div>
									
									<div class="masonry-news-list_meta-info clearfix">
										<div class="meta">
											<div class="views"><span class="icon" href="#"></span> <span>265</span></div>
											<div class="comments">
												<a class="icon" href="#"></a>
												<a href="">152</a>
											</div>
										</div>
										
										<a href="" class="textdec-onhover">Добавить <br />комментарий</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Изменила цель </p>
										</div>
									</div>
									<div class="user-purpose">
									    <i class="icon"></i>
									    <div class="purpose-container">
										<span>Цель №1</span><p>На море с дочкой летом</p>
										</div>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Изменила статус </p>
										</div>
									</div>
									<div class="clearfix user-info-big">
										<div class="text-status">
									        <p><a href="" class="textdec-onhover">Родители всегда говорили мне, что я могу добиться чего угодно! Родители всегда говорили мне...</a></p>
									        <span class="tale"></span>
									        <div class="meta nofloat">
												<span class="views"><span class="icon" href="#"></span> <span>265</span></span>
												<span class="comments">
													<a class="icon" href="#"></a>
													<a href="">15233</a>
												</span>
											</div>
									    </div>
									</div>
									<div class="comments-all clearfix">
										<a href="">Добавить комментарий</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Алекса</a>
											<span class="date">Сегодня 13:25</span>
											<p>Участвует в фотоконкурсе </p>
										</div>
									</div>
									<div class="contest-participant">
										<img class="title-img" alt="" src="/images/broadcast/title-contest-4.jpg">
										<div class="img">
											<a href="">
												<img src="/images/example/gallery_album_img_01.jpg">
												<span class="btn">Посмотреть</span>
											</a>
										<div class="item-title">Разнообразие десертов сицилийского стиля</div>
										</div>
										<div class="clearfix">
											<div class="position">
												<strong>18</strong> место
											</div>
											<div class="ball">
												<div class="ball-count">186</div>
												<div class="ball-text">баллов</div>
											</div>
										</div>
									</div>
									<div class="comments-all">
										<a href="" class="textdec-onhover">Поддержите друга!</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Вступила в клубы</p>
										</div>
									</div>
									<div class="clubs-list">
										<ul>
											<li class="club-img rest">
				                            	<a href="/community/19/forum/">
				                                    <img src="/images/club_img_33.png">
				                                    Выходные с ребенком
				                                </a>
				                                <a href="" class="club-join">Вступить</a>
				                            </li>
				                            <li class="club-img beauty">
				                            	<a href="/community/19/forum/">
				                                    <img src="/images/club_img_21.png">
				                                    Путешествия семьей
				                                </a>
				                                <a href="" class="club-join">Вступить</a>
				                            </li>
										</ul>
									</div>
									<div class="comments-all clearfix">
										<a href="">Добавить комментарий</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Вступила в клубы</p>
										</div>
									</div>
									<div class="clubs-list">
										<ul>
											<li class="club-img rest">
				                            	<a href="/community/19/forum/">
				                                    <img src="/images/club_img_33.png">
				                                    Выходные с ребенком
				                                </a>
				                                <a href="" class="club-join">Вступить</a>
				                            </li>
				                            <li class="club-img beauty">
				                            	<a href="/community/19/forum/">
				                                    <img src="/images/club_img_21.png">
				                                    Путешествия семьей
				                                </a>
				                                <a href="" class="club-join">Вступить</a>
				                            </li>
				                              <li class="club-img beauty">
				                            	<a href="/community/19/forum/">
				                                    <img src="/images/club_img_26.png">
				                                    Путешествия семьей
				                                </a>
				                                <a href="" class="club-join">Вступить</a>
				                            </li>
										</ul>
									</div>
									<div class="comments-all clearfix">
										<a href="">Добавить комментарий</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Вступила в клубы</p>
										</div>
									</div>
									
									<div class="user-interests">
										<ul class="interests-list">
	                                        <li><span class="interest">Фототуризм</span></li>
	                                        <li><span class="interest">Кино</span></li>
	                                        <li><span class="interest">Водный туризм</span></li>
	                                        <li><span class="interest">Рок</span></li>
	                                        <li><span class="interest">Водный туризм</span></li>
	                                    </ul>
									</div>
									
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила фото в альбом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука</a>
										<a href="" class="icon-photo"></a>
									</h3>
									<div class="masonry-news-list_content">
										<ul class="masonry-news-list_img-list clearfix">
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
										</ul>
									</div>
									<div class="comments-all clearfix">
										<a href="">Смотреть весь альбом</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила фото в альбом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука</a>
										<a href="" class="icon-photo"></a>
									</h3>
									<div class="masonry-news-list_content">
										<ul class="masonry-news-list_img-list clearfix">
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
										</ul>
									</div>
									<div class="comments-all clearfix">
										<a href="">Смотреть весь альбом</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила фото в альбом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука</a>
										<a href="" class="icon-photo"></a>
									</h3>
									<div class="masonry-news-list_content">
										<ul class="masonry-news-list_img-list clearfix">
											<li><a href=""><img src="/images/example/w64-h61-1.jpg" alt="" /></a></li>
											<li><a href=""><img src="/images/example/w64-h61-2.jpg" alt="" /></a></li>
										</ul>
									</div>
									<div class="comments-all clearfix">
										<a href="">Смотреть весь альбом</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила фото в альбом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Волшебная азбука</a>
										<a href="" class="icon-photo"></a>
									</h3>
									<div class="masonry-news-list_content">
										<div class="title-img">
											<a href="">
												<img alt="" src="/images/example/gallery_album_img_10.jpg">
												<span class="btn-view">Посмотреть</span>
											</a>
											
										</div>
									</div>
									<div class="comments-all clearfix">
										<a href="">Смотреть весь альбом</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила комментарий </p>
										</div>
									</div>
									<div class="masonry-news-list_comment">
										<div class="masonry-news-list_comment-text">
											Проблема в том, что на 2006-ой год игры еще толком не было, Роды — отправная точка для самых важных изменений и судьбоносных. Осень. Я ее люблю, но когда начинается непонятная погода, хочется лезть на стенку. Сегодня на улице вот +8, вроде тепло, но вот туман.
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Пасха со сгущенкой</a>
									</h3>
									<div class="clearfix">
										<a href="" class="sub-category"><span class="icon-cook"></span>Кулинарный рецепт</a>
									</div>
									<div class="masonry-news-list_meta-info clearfix">
										
										<div class="meta">
											<div class="views"><span class="icon" href="#"></span> <span>11265</span></div>
											<div class="comments">
												<a class="icon" href="#"></a>
												<a href="">155</a>
											</div>
										</div>
										
										<a href="" class="textdec-onhover">Вступить <br />в беседу!</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила <a href="" class="sub-category"><span class="icon-cook"></span>Кулинарный рецепт</a></p>
										</div>
									</div>
									<h3 class="masonry-news-list_title">
										<a href="">Пасха со сгущенкой</a>
									</h3>
									<div class="masonry-news-list_content">
										<div class="title-img">
											<a href="">
												<img src="/images/example/gallery_album_img_10.jpg" alt="">
												<span class="btn-view">Посмотреть</span>
											</a>
											
										</div>
										<p>Роды — отправная точка для самых важных изменений и судьбоносных... <a href="" class="all">Весь рецепт</a></p>
									</div>
									
									<div class="masonry-news-list_meta-info clearfix">
										<div class="meta">
											<div class="views"><span class="icon" href="#"></span> <span>265</span></div>
											<div class="comments">
												<a class="icon" href="#"></a>
												<a href="">152</a>
											</div>
										</div>
										
										<a href="" class="textdec-onhover">Добавить <br />комментарий</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Воспользовалась сервисом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title textalign-c">
										<a href="">Калькулятор мер</a>
									</h3>
									<div class="masonry-news-list_content">
										<a href="">
											<img alt="" src="/images/services/service-icon-12.jpg">
										</a>
									</div>
									<div class="comments-all clearfix">
										<a href="">Попробовать сервис!</a>
									</div>
								</li>
								
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Воспользовалась сервисом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title textalign-c">
										<a href="">Калькулятор мер</a>
									</h3>
									<div class="masonry-news-list_content">
										<a href="">
											<img alt="" src="/images/services/service_img_12.png">
										</a>
									</div>
									<div class="comments-all clearfix">
										<a href="">Попробовать сервис!</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Воспользовалась сервисом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title textalign-c">
										<a href="">Счетчик калорий</a>
									</h3>
									<div class="masonry-news-list_content">
										<a href="">
											<img alt="" src="/images/services/service-icon-13.jpg">
										</a>
									</div>
									<div class="comments-all clearfix">
										<a href="">Попробовать сервис!</a>
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Добавила семью </p>
										</div>
									</div>
									<div class="textalign-c clearfix">
										<a href=""><img src="/images/user-family.png" alt="" /></a>
									</div>
									<div class="masonry-news-list_content">
									
										<div class="user-family">
											<ul>
												<li>
													<big>Дмитрий <small>- мой муж</small></big>
													<div class="img">
														<img src="http://img.happy-giraffe.ru/thumbs/180x180/15292/1916d87aa7c17e09756746f0854e52e2.jpg">
													</div>
												</li>
												<li>
												<big>Мирослава <small>- моя дочь, 1 год 2 месяца</small></big>
												<div class="img">
													<img src="http://img.happy-giraffe.ru/thumbs/180x180/15292/a1838cb82b9b6022801e25cc4ac10ec3.jpg">
												</div>
												</li>
												<li class="waiting clearfix">
													<i class="icon"></i>
													<div class="in">
														<big>Ждем еще</big>
														<div class="gender">Девочку <i class="icon-female"></i></div>
														<div class="time">7-я неделя</div>
													</div>
												</li>
											</ul>

											<a href="/user/15292/albums/3139/" class="watch-album">Смотреть семейный<br>альбом</a>        
										</div>
										
									</div>
								</li>
								
								<li class="masonry-news-list_item">
									<div class="masonry-news-list_friend-info clearfix">
										<a class="ava female small" href=""><img alt="" src="http://img.happy-giraffe.ru/avatars/19372/small/21fd8c8f29c5a289df17938cb2f20e5b.jpg"></a>
										<div class="details">
											<a class="username" href="">Александр</a>
											<span class="date">Сегодня 13:25</span>
											<p>Воспользовалась сервисом </p>
										</div>
									</div>
									<h3 class="masonry-news-list_title textalign-c">
										<a href="">Приправы и специи</a>
									</h3>
									<div class="masonry-news-list_content">
										<a href="">
											<img alt="" src="/images/services/service-icon-19.jpg">
										</a>
									</div>
									<div class="comments-all clearfix">
										<a href="">Попробовать сервис!</a>
									</div>
								</li>
								
								
							</ul>
						</div>
				

				</div>

			</div>
		
		</div>
		<a href="#layout" id="btn-up-page"></a>
		<div class="footer-push"></div>
	</div>
	<?php include $_SERVER['DOCUMENT_ROOT'].'/block/global/footer.php'; ?>
</div>
	

</body>
</html>
...
$(window).load(function() {
    ...
    if (im.length > 0) {
        ...
        var messagesViewHeight = $(window).height() - $('.im-center_top').height() - $('.im-center_bottom').height() - headerHeight - 60; 
        ...
        $('.layout-container').scroll(function () {
            ...
            if (contanerScroll > headerHeight ) {
                ...
                var messageTopScroll = -messagesHeight + messagesViewHeight + contanerScroll  ;
                 massageWrapper.css('top', messageTopScroll );
                 imSidebarHeight(contanerScroll, headerHeight);
                 /* заглушка; 53 - высота top-line-menu + отступ внутри .im 11px */
                 $('.im-cap').css('top', $('#header-new').height() + 53 - contanerScroll);
            }
...
function imSidebarHeight(contanerScroll, headerHeight) {
    ...
    if (im__fixed.length > 0) {
        $('.im-user-list').height($('.im-sidebar').height() - 198);
    } else {
        $('.im-user-list').height($(window).height() - headerHeight - 189 + contanerScroll);
    }

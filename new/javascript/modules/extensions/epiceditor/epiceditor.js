...
define(function () {
  ...
  EpicEditor.prototype._setupTextareaSync = function () {
    ...
    EpicEditor.prototype._setupHTMLSync = function () {
        var self = this
            , htmlareaFileName = self.settings.file.name
            , _setupHTMLSync;
        ...
        // creating two timers now in some configurations. We keep the textarea synced
        // by saving and opening the textarea content from the draft file storage.
        self._htmlareaSaveTimer = window.setInterval(function () {
            if (!self._canSave) {
                return;
            }
            self.save(true);
        ...
        _HTMLSync = function () {
            ...
            // the fact draft files haven't been saved by the time this is called.
            // TODO: Add test for this case.
            self._htmlareaElement.value = self.exportFile(htmlareaFileName, 'html', true) || self.settings.file.defaultContent;
        }

        if (typeof self.settings.html == 'string') {
            self._htmlareaElement = document.getElementById(self.settings.html);
        }
        else if (typeof self.settings.html == 'object') {
            self._htmlareaElement = self.settings.html;
        }
        ...
        // If the developer wants drafts to be recoverable they should check if
        // the local file in localStorage's modified date is newer than the server.
        if (self._htmlareaElement.value !== '') {
            //self.importFile(htmlareaFileName, self._htmlareaElement.value);

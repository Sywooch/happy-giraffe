...
class m120531_135339_recipes_update extends CDbMigration
{
	public function up()
	{
        ...
        $this->execute("
DROP TABLE IF EXISTS `cook__cuisines`;
DROP TABLE IF EXISTS `cook__recipes`;
DROP TABLE IF EXISTS `cook__recipe_ingredients`;
DROP TABLE IF EXISTS `cook__recipe_steps`;
        ");
...
ALTER TABLE `cook__recipes`
  ADD CONSTRAINT `cook__recipes_ibfk_3` FOREIGN KEY (`author_id`) REFERENCES `users` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
ALTER TABLE `cook__recipe_ingredients`
  ...
  ADD CONSTRAINT `cook__recipe_ingredients_ibfk_3` FOREIGN KEY (`unit_id`) REFERENCES `cook__units` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
	...
	{
		echo "m120531_135339_recipe_update does not support migration down.\n";
		return false;
	}

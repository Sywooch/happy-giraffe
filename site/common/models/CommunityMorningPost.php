...
class CommunityMorningPost extends HActiveRecord
{
    ...
    public function beforeSave()
    {
        if ($this->isNewRecord){
            ...
            $today_contents = CommunityContent::model()->with('morning')->findAll('created > "'.date("Y-m-d").' 00:00:00" AND type_id = 4');
            $pos = 0;
            foreach($today_contents as $today_content){
                if (isset($today_content->morning) && $today_content->morning->position > $pos)
                    $pos = $today_content->morning->position;
            }
            $this->position = $pos + 1;
        }

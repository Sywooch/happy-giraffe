...
Yii::import('site.frontend.extensions.image.Image_Driver');
...
class Image {
	...
	public function __construct($image, $config = NULL)
	{
        ...
        // Load the driver
        Yii::import("site.frontend.extensions.image.drivers.$driver");
	...
	/**
	 * @param Image $image
	 * @param type $transparency default 80
	 * @param type $left default null
	 * @param type $top default null
	 * @return Image 
	 */
	public function watermark(Image $image, $transparency = 80, $left = null, $top = null)
	{
		if(is_null($left))
		{
			$left = $this->width - $image->width - 10;
		}

		if(is_null($top))
		{
			$top = $this->height - $image->height - 10;
		}
	...
	public function accurate_resize($width, $height)
	{
		if ( ! $this->valid_size('width', $width))
			throw new CException('image invalid width');
		...
		$master = ($ratio > $new_ratio) ? Image::HEIGHT : Image::WIDTH;
	...
	public function product_resize($width, $height)
	{
		if ( ! $this->valid_size('width', $width))
			throw new CException('image invalid width');
		...
		$ratio = $this->width / $this->height;
		$new_ratio = $width / $height;
		$master = ($ratio > $new_ratio) ? Image::WIDTH : Image::HEIGHT;

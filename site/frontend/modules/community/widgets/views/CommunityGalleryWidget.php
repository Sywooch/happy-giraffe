...
<div class="gallery-widget">
    ...
    <a href="javascript:void(0)" class="gallery-widget_img" onclick="PhotoCollectionViewWidget.open(<?=CJavaScript::encode('PhotoPostPhotoCollection')?>, <?=CJavaScript::encode(array('contentId' => $widgetToShow->gallery->content->id))?>, <?=CJavaScript::encode($widgetToShow->item->photo_id)?>, <?=CJavaScript::encode(array('exitUrl' => $widgetToShow->gallery->content->getUrl()))?>)">
        <?=CHtml::image($widgetToShow->item->photo->getPreviewUrl(580, null, Image::WIDTH), $widgetToShow->gallery->content->title)?>
    ...
    <div class="gallery-widget_bottom clearfix">
        <a href="javascript:void(0)" class="float-r btn-blue-light btn-medium" onclick="PhotoCollectionViewWidget.open(<?=CJavaScript::encode('PhotoPostPhotoCollection')?>, <?=CJavaScript::encode(array('contentId' => $widgetToShow->gallery->content->id))?>, <?=CJavaScript::encode($widgetToShow->item->photo_id)?>, <?=CJavaScript::encode(array('exitUrl' => $widgetToShow->gallery->content->getUrl()))?>)">Смотреть галерею</a>
        <div class="meta-gray">

<div class="contest-about clearfix">
    ...
    <div class="sticker">
        ...
        <?php if(!ContestWork::model()->findByAttributes(array('user_id' => Yii::app()->user->id, 'contest_id' => $this->contest->id))): ?>
            <center><a href="<?=(Yii::app()->user->isGuest) ? '#login' : $this->createUrl('/contest/statement', array('id' => $this->contest->id))?>" class="btn btn-green-medium<?=(Yii::app()->user->isGuest) ? ' fancy' : ''?>"><span><span>Участвовать<i class="arr-r"></i></span></span></a></center>
        <?php endif; ?>
    </div>

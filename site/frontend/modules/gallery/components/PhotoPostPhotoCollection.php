...
class PhotoPostPhotoCollection extends PhotoCollection
{
    ...
    {
        $dependency = new CDbCacheDependency("SELECT COUNT(*), MAX(i.id) FROM community__content_gallery_items i INNER JOIN community__content_gallery g ON i.gallery_id = g.id WHERE g.content_id = :content_id");

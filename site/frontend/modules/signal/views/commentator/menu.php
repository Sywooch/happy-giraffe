...
?><div class="nav-hor nav-hor__users clearfix">
    <ul class="nav-hor_ul">
        <li class="nav-hor_li<?php if ($active == null) echo ' active' ?>">
            <a href="<?=$this->createUrl('/signal/commentator/'.$url, array('month'=>$month)) ?>" class="nav-hor_i">
        ...
        <?php if (!isset($do_not_show_list) || $do_not_show_list === false):?>
            ...
            <?php foreach ($entities as $entity): ?>
                ...
                <li class="nav-hor_li<?php if ($entity == $active) echo ' active' ?>">
                    <?php $_url = $this->createUrl('/signal/commentator/'.$url, array('month'=>$month, 'user_id'=>$user->id))?>
                    <a class="nav-hor_i" href="<?=$_url ?>">
                        <span class="ava small"><?=CHtml::image($user->getAvatarUrl(24)) ?></span>

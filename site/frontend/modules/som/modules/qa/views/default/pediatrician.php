...
<div class="b-col b-col--6 b-col-sm--10 b-col-xs">
    <div class="b-mobile-nav">
        ...
        <div class="b-mobile-nav__right">
            <a id="addNewQuestionBtn" href="<?=$this->createUrl("/som/qa/default/pediatricianAddForm/")?>" class="disabled b-mobile-nav__btn btn btn--default login-button" data-bind="follow: {}">Задать вопрос</a>
        </div>
    ...
    <div class="b-nav-panel">
        ...
        $queryFilter = !empty($filter) ? ['filter' => $filter] : [];
        ?>
            ...
            <div class="b-filter-menu b-filter-menu--theme-default">
                ...
                <ul class="b-filter-menu__list">
                    <li class="b-filter-menu__item">
                        <a href="<?=UrlCreator::create('mypediatrician/', $queryFilter )?>" class="b-filter-menu__link <?=$tab == $this::TAB_NEW ? 'b-filter-menu__link--active' : ''?>"><?=$tabTitle[$this::TAB_NEW]?></a>
                    </li>
                    <li class="b-filter-menu__item">
                        <a href="<?=UrlCreator::create('mypediatrician/' . $this::TAB_UNANSWERED, $queryFilter)?>" class="b-filter-menu__link <?=$tab == $this::TAB_UNANSWERED ? 'b-filter-menu__link--active' : ''?>"><?=$tabTitle[$this::TAB_UNANSWERED]?></a>
                    </li>
                    <li class="b-filter-menu__item">
                        <a href="<?=UrlCreator::create('mypediatrician/' . $this::TAB_All, $queryFilter)?>" class="b-filter-menu__link <?=$tab == $this::TAB_All ? 'b-filter-menu__link--active' : ''?>"><?=$tabTitle[$this::TAB_All]?></a>
                    </li>
                </ul>
            </div>
        </div>

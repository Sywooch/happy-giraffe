<div class="like-btn">
     <?php if (Yii::app()->user->isGuest):?>
         <a class="btn-icon heart heart2 popup-a" href="#loginWidget" data-theme="white-square"></a>
     <?php else: ?>
         <a class="btn-icon heart heart2<?=(! Yii::app()->user->isGuest && get_class($this->model) == 'ContestWork' && Yii::app()->user->model->score->full == 0) ? ' yohoho_steps':''?><?php echo isset($this->model->author) && Yii::app()->user->id == $this->model->author->id ? ' yohoho_me ' : '' ?><?php echo HGLike::model()->findByEntity($this->model) ? ' active' : ''; ?>" href="javascript:;" onclick="pushYohoho(this);"></a>
        <?php if (! Yii::app()->user->isGuest): ?>

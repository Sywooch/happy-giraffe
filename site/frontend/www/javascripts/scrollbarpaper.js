...
(function ($) {
    ...
    var getState = function (obj) {
        ...
        return{height:height, ratio:height / (data.clearer.position().top - data.content.position().top), offset:obj.offset()}
    };
    var methods = {_init:function () {
        ...
        return this.each(function () {
            ...
            if (getState($this).ratio < 1) {
                ...
                var state = getState($this);
                data.paper.css({left:-parentOffset.left + state.offset.left + $this.innerWidth() - data.paper.width(), top:-parentOffset.top + state.offset.top});
